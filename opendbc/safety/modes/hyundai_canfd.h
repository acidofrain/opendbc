...
static safety_config hyundai_canfd_init(uint16_t param) {
  ...
  // SCC_CONTROL (0x1A0) is added here, not in the COMMON macro, because the LONG variant below
  // already carries its own SCC_CONTROL(..., true) -- baking it into COMMON would double-declare
  // 0x1A0 with conflicting check_relay values in that array. This non-longitudinal entry is what
  // lets a cancel-only SCC_CONTROL send (see create_acc_cancel) through the TX allow-list for
  // ALT_BUTTONS cars that don't run openpilotLongitudinalControl (e.g. Carnival 2025+).
